...
Claude Code 多源 AI 技术新闻摘要——从 12 个来源抓取、汇总最新 AI 技术动态，并通过学习层提供智能分析。
...
**步骤 1 — 通知 & 抓取**：输出预计耗时提示（新闻约 20–40 秒，含学习层额外 +60–80 秒），然后通过单次 Bash 调用从 12 个来源抓取内容：
...
| `devto` | DEV Community AI 文章（tag:ai，热反应排序） |
...
| `--sources` | 逗号分隔的来源 ID（见上表） | 全部 12 个来源 |
| `--limit N` | 每个来源最多条数 | 5 |
...
│   ├── DESIGN.md               # 数据源配置及设计说明（不加载到运行时上下文）
│   └── scripts/
...
每次调用生成唯一的 `ND_SESSION` token，所有 `/tmp` IPC 文件均以 `/tmp/nd_{session}_*` 命名空间隔离，多个 `/news-digest` 并发执行互不干扰。

```
/news-digest（命令协调者）
│
├── 步骤 0：生成 ND_SESSION → Bash — 检测项目 profile → /tmp/nd_{session}_profile.json
│           解析参数 → /tmp/nd_{session}_params.json（topics、sources、limit、no_learn）
├── 步骤 1：输出启动提示（预计耗时）→ Bash: curl 所有来源 → /tmp/nd_{session}_*.{json,xml,html}
├── 步骤 2：Bash — Python heredoc：解析 → 过滤 → 去重 → 相关性标记
│           写入 /tmp/nd_{session}_deduped.json + /tmp/nd_{session}_relevant.json
│           （>50% 来源失败时 ⚠️ 告警；>3 个失败时 [诊断] 警告）
├── 步骤 3：输出格式化 CLI 摘要
│
└── 步骤 4（相关条目存在且未设 --no-learn 时）：
    └── news-learner（agent）← 接收 relevant_items_path、project_profile、learner_instruction
