...
items = json.loads('{relevant_items}')  # 协调者替换为实际 JSON 字符串
...
> `{relevant_items}` 由协调者在传入 prompt 中替换为实际 JSON 数组字符串（如 `[{"title":"...","url":"..."}]`）。单引号 heredoc 标签防止 bash 展开，JSON 内容由 Python3 安全解析，不依赖 bash 变量展开。
...
- 输入 `w`：**news-learner agent 直接将该条目降级为 `[Learn]` 并写入 tech-watch.md**（格式同 Step 6 [Learn] 归档），无需协调者介入
