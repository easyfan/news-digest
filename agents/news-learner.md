...
relevant_items_path: /tmp/nd_relevant.json
  # 实际数据通道：协调者 Step 2 已将 relevant_items 写入该文件（JSON 数组，每条含 {title, url, source, snippet, metric}）
  # news-learner Step 2 直接 json.load() 读取，不依赖 prompt 中的内联 JSON 参数
...
> `PLATFORM_ROOT` 赋值后用于 glob 展开（`"$PLATFORM_ROOT"/commands/*.md`），变量加双引号防止路径含空格时分词错误，glob 符号置于引号外确保 bash 正确展开。
...
# 只读最近 150 行（约 30 条条目），控制 token 消耗；历史过长时早期条目不参与去重判断，可接受
...
> **scratch 文件生命周期**：scratch 文件为临时分析产物，存储于 `{PROJECT_ROOT}/.claude/agent_scratch/`，同一天多次运行会覆盖同名文件（名称含日期不含时间戳）。tech-watch.md 中的"详细分析"字段记录 scratch 路径，但该路径长期不保证有效；重要的 gap 分析内容已内嵌在 tech-watch.md 条目中。
...
筛选推荐等级为 `[Learn]` 的条目，**写入前先检查 URL 是否已在 tech-watch.md 中存在**（避免同一新闻多次被识别时重复归档）：

```bash
# URL 去重检查 + 追加写入：对每条 [Learn] 条目，先检查 URL 是否已在 tech-watch.md 中，再追加
TECH_WATCH="$PLATFORM_ROOT/projects/$CURRENT_PROJECT/memory/tech-watch.md"
python3 - "$TECH_WATCH" "$ITEM_URL" "$ITEM_ENTRY" << 'LEARN_ARCHIVE_EOF'
import sys, os, pathlib

tech_watch_path = sys.argv[1]
item_url = sys.argv[2]
item_entry = sys.argv[3]  # 已格式化的归档条目文本（含 ## 标题行至 --- 分隔符）
...
调用上方 Python 块前，先将 `ITEM_ENTRY` 构造为以下格式（每条一个条目块，用 `---` 分隔）：
...
若本次无 `[Learn]` 条目，跳过归档步骤，不调用 Python 块，不写入文件。
...
输入语法：
  <编号>     立即执行该条采纳方案（如 1、2）
  s <编号>   跳过指定条目并记入历史（如 s 1）
  s all      跳过全部并记入历史
  w <编号>   将指定条目降级为 [Learn] 归档到 tech-watch 观察（如 w 2）
...
- 输入 `s N` / `s all`：条目标记为 `skipped`，记入历史，不归档
- 输入 `w N`：**由协调者（news-digest）处理**，将条目降级为 `[Learn]` 归档到 tech-watch.md（格式同 Step 6 [Learn] 归档）

> **职责边界**：以上响应由协调者（/news-digest command）读取并处理。news-learner 本身仅输出决策提示文本，不直接执行任何工具调用。
...
---
完整分析已保存至: {scratch}
[Learn] 条目已归档至: {platform_root}/projects/{current_project}/memory/tech-watch.md（若本次无 [Learn] 条目则省略此行）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
