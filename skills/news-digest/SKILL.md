...
SKILL_DIR="$PLATFORM_ROOT/skills/news-digest"
PROJECT_ROOT=$(pwd)
...
```bash
python3 "$SKILL_DIR/scripts/parse_arguments.py" "$ARGUMENTS"
```
...
```bash
python3 "$SKILL_DIR/scripts/detect_project_profile.py" "$PROJECT_ROOT"
```
...
```bash
bash "$SKILL_DIR/scripts/fetch_sources.sh"
```
...
```bash
python3 "$SKILL_DIR/scripts/parse_items.py"
```
...
```bash
python3 "$SKILL_DIR/scripts/filter_items.py"
```
...
python3 "$SKILL_DIR/scripts/archive_learn.py" "${ITEM_IDX}" "$TECH_WATCH"
```
